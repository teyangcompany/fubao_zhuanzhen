        ...
        <form class="login-form clearfix">
          ...
          <div class="clearfix login-tit  login-item-wrap " style="margin-top:50px;">
            ...
            <input type="text" style="width:90%;" class="login-input h34  fl"
                   placeholder="请输入手机号" v-model="loginForm.docMobile" required>
          </div>
          <div class="clearfix login-tit  login-item-wrap" style="margin-top:30px;">
            ...
            <input type="password" style="width:90%;" class="login-input h34  fl"
                   placeholder="请输入密码" v-model="loginForm.docPassword" required>
          </div>
  ...
  export default {
    ...
    methods: {
      async submitForm() {
        ...
        form.docPassword = sha512(hex_md5(form.docPassword) + form.docPassword)
        this.loading = true;
        let ret = await http("nethos.doc.login", form);
        this.loading = false;
        if (ret.code == 0) {
          let user = ret.obj, token = ret.token;
